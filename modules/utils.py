...
def plot_improved_predictions(images, targets, predictions, similarities, classnames, k=3):
    """
    Plot the top-k images where the model shows the best improvement in predictions,
    measured by the difference between the similarity with the correct class and
    the previously highest predicted class.
    
    The improvement is calculated as: similarity of true class - highest similarity among other classes.
    ...
    for idx in range(len(predictions)):
        ...
        # Get similarity for the true class
        true_sim = similarities[idx, true_class]
        
        # Get the highest similarity excluding the true class
        other_sims = similarities[idx].copy()
        other_sims[true_class] = -float('inf')  # Exclude true class
        max_other_sim = np.max(other_sims)
        max_other_class = np.argmax(other_sims)
        
        # Calculate improvement (delta between true class similarity and highest other similarity)
        improvement = true_sim - max_other_sim
        
        improvements.append({
            'index': idx,
            'improvement': improvement,
            'true_sim': true_sim,
            'max_other_sim': max_other_sim,
            'true_class': true_class,
            'max_other_class': max_other_class,
    ...
    title_ax = fig.add_subplot(gs[0])
    title_ax.axis('off')
    title_ax.text(0.5, 0.5, "Images with Greatest Improvement in Prediction Confidence", 
    ...
    for i, result in enumerate(top_k):
        idx = result['index']
        label_text = (
            f"True: {classnames[result['true_class']]}\n"
            f"Prev Best: {classnames[result['max_other_class']]}\n"
            f"Current: {classnames[result['prediction']]}\n"
            f"Improvement: {result['improvement']:.3f}\n"
            f"(True: {result['true_sim']:.3f} vs\n"
            f"Other: {result['max_other_sim']:.3f})"
        )
        
        axes[i].imshow(images_display[idx])
        axes[i].axis('off')
        axes[i].set_title(label_text, fontsize=8, pad=5)
